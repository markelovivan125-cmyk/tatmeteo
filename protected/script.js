(function() {
  ...
  try { var rsv = localStorage.getItem('radarSource'); if (rsv === 'rainradar' || rsv === 'nowcast') radarSource = rsv; else { radarSource = 'nowcast'; try { localStorage.setItem('radarSource', 'nowcast'); } catch (e) {} } } catch (e) {}
  ...
  function setSatProduct(id) {
    ...
    if (prev) prodExit();
    if (id) {
      prodEnter(); toast('Продукт: ' + prodDef().label);
    } else {
  ...
  function setRadarSource(src, auto) {
    if (src !== 'nowcast' && src !== 'rainradar') src = 'nowcast';
    radarSource = src;
  ...
  function setLayer(newLayer) {
    ...
    if (S.layer === 'dop') {
      ...
      buildFrames(); updHUD();
      dopStart(); toast('🌀 Доплер: радиальная скорость ДМРЛ (4x4)');
  ...
  function buildLegend() {
    ...
    if (isRadarLegend) {
      ...
      srcBtn.addEventListener('click', function() { var nx = radarSource === 'nowcast' ? 'rainradar' : 'nowcast'; setRadarSource(nx, false); });
