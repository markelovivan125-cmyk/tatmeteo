(function() {
  ...
  function updateRulerLabels() {
    ...
    for (var i = 1; i < S.ruler.points.length; i++) {
      totalDist += S.ruler.points[i - 1].distanceTo(S.ruler.points[i]);
    }
    var distKm = (totalDist / 1000).toFixed(1);

    var midIdx = Math.floor((S.ruler.points.length - 1) / 2);
    var midLatLng = S.ruler.points[midIdx];
    var labelHtml = '<div class="ruler-label">📏 ' + distKm + ' км</div>';
    S.ruler.label = L.marker(midLatLng, {
    ...
    for (var j = 1; j < S.ruler.points.length; j++) {
      ...
      var segLabel = L.marker(midSeg, {
        icon: L.divIcon({
          className: '',
          html: '<div class="ruler-segment-label">' + segDist + ' км</div>',
          iconSize: [0, 0],
          iconAnchor: [0, 0]
        })
      }).addTo(map);
      S.ruler.segmentLabels.push(segLabel);
    }
  }
  ...
  function getBlockAt(mx, my) {
    ...
    for (var i = 0; i < tiles.length; i++) {
      ...
      var oxF = Math.max(0, Math.min(255.9, (mx - r.sx) / r.sw * 256));
      var oyF = Math.max(0, Math.min(255.9, (my - r.sy) / r.sh * 256));
      var bx = Math.floor(oxF / effectivePx) * effectivePx;
      var by = Math.floor(oyF / effectivePx) * effectivePx;
      var bx2 = Math.min(bx + effectivePx, 256);
      var by2 = Math.min(by + effectivePx, 256);
      var cx2 = (bx + bx2) / 2 | 0;
      var cy2 = (by + by2) / 2 | 0;
